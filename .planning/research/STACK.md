...
**Confidence:** HIGH (all versions fetched live from PyPI/npm/GitHub)
...
uv add fastapi==0.135.1 uvicorn[standard]==0.42.0
uv add sqlalchemy==2.0.48 asyncpg==0.31.0 alembic==1.18.4
uv add pydantic==2.12.5 pydantic-settings
uv add "mcp>=1.26,<2"
uv add sentence-transformers==5.3.0
uv add openai anthropic httpx==0.28.1

# Python dev dependencies
uv add --dev pytest pytest-asyncio==1.3.0

# Frontend (via pnpm, requires Node >=20.19.0)
pnpm create vite@8 frontend --template react-ts
cd frontend
...
**If running fully local (no API keys configured):**
...
**If BYOK OpenAI keys are configured:**
...
**If BYOK Anthropic keys are configured:**
...
**If Ollama is configured (high-privacy mode):**
...
**If pgvector query latency exceeds target (P95 > 2s at 100k items):**
...
---
*Stack research for: Recalium — local-first MCP-native personal memory platform*
*Researched: 2026-03-22*
