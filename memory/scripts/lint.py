...
Usage:
    ...
    uv run python scripts/lint.py --structural-only  # skip LLM checks (faster)
"""
...
from config import KNOWLEDGE_DIR, MODEL_LINT, REPORTS_DIR, now_iso, today_iso
from utils import (
    count_inbound_links,
    extract_wikilinks,
    file_hash,
...
def check_orphan_sources() -> list[dict]:
    """Check for daily logs that have not been compiled yet."""
    state = load_state()
...
def check_contradictions() -> list[dict]:
    ...
    prompt = f"""Review this knowledge base for contradictions, inconsistencies,
or conflicting claims across articles.
...
Do NOT output anything else."""
    ...
    response = ""
    try:
        response = chat(MODEL_LINT, prompt, max_tokens=1024)
