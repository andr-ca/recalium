...
| RR-010 | MCP resources/evidence | **PARTIAL — PO decision 2026-08-26:** MCP *resources* deferred to M3 (ADR 0001 timeline); M1 remainder is live-client *evidence* only (schemas, invalid inputs, audit metadata, concurrent SSE clients). Tools + error envelope closed (RR-008/009). | “Fully working MCP” claim weak until live-client evidence lands. | M1: add live-client tests + evidence doc. M3: implement MCP resources if still desired. |
