...
| RR-009 | MCP error contract | ✓ CLOSED — Stable error envelope implemented | All MCP tool responses return `{status: "error", error: {code, message, details, retryable}}` for errors; validation, internal, idempotency conflict, and not-found codes distinguish error types; all errors carry retryable flag. | ✓ Evidence: backend/app/mcp_server/server.py:33-52 (_mcp_error function), backend/tests/mcp/test_mcp_server.py:73-118 (error envelope tests). |
